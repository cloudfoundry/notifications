...
var _ = Describe("NotifyRouter", func() {
	...
	BeforeEach(func() {
		router = web.NewNotifyRouter(fakes.NewNotify(), fakes.NewErrorWriter(), fakes.NewStrategy(), middleware.RequestLogging{}, middleware.Authenticator{Scopes: []string{"notifications.write"}}, middleware.DatabaseAllocator{}, fakes.NewStrategy(), fakes.NewStrategy(), fakes.NewStrategy(), fakes.NewStrategy(), fakes.NewStrategy(), middleware.Authenticator{Scopes: []string{"emails.write"}})
	})
