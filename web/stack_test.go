...
var _ = Describe("Stack", func() {
    It("assigns the Handler as the last object passed to the constructor", func() {
        handler := Handler{}
        stack := web.NewStack(handler).Use(Middleware{})
        Expect(stack.Handler).To(Equal(handler))
    })

    It("assigns the remaining handlers as middleware", func() {
        middleware1 := Middleware{}
        middleware2 := Middleware{}
        stack := web.NewStack(Handler{}).Use(middleware1, middleware2)
        Expect(len(stack.Middleware)).To(Equal(2))
        Expect(stack.Middleware).To(ContainElement(middleware1))
        Expect(stack.Middleware).To(ContainElement(middleware2))
    })

    It("executes the stack in order", func() {
        stack := web.NewStack(Handler{}).Use(Middleware{"first"}, Middleware{"second"})
        ...
        stack.ServeHTTP(writer, request)
    ...
    It("stops cascading the handlers if a middleware handler returns false", func() {
        stack := web.NewStack(Handler{}).Use(Middleware{"first"}, HaltingMiddleware{"second"})
        writer := httptest.NewRecorder()
        request, err := http.NewRequest("GET", "/my/path", nil)
        if err != nil {
            panic(err)
    ...
    Describe("handles concurrent requests", func() {
        It("ensures the headers, code, and body of a response are unique to a request", func() {
            ...
            stack := web.NewStack(ChannelHandler{finish, done}).Use(Middleware{"concurrent"})
            ...
            go func() {
                stack.ServeHTTP(writer1, request1)
            }()

            go func() {
                stack.ServeHTTP(writer2, request2)
            }()
