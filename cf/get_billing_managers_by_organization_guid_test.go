...
var _ = Describe("GetBillingManagersByOrgGuid", func() {
    ...
    BeforeEach(func() {
        ...
        CCServer = httptest.NewServer(BillingManagersEndpoint)
    ...
    It("returns a list of billing managers for the given organization guid", func() {
        users, err := cloudController.GetBillingManagersByOrgGuid(testOrganizationGuid, testUAAToken)
        if err != nil {
            panic(err)
        }
    ...
    It("returns an error when the Cloud Controller returns a 400, or 500 status code", func() {
        _, err := cloudController.GetBillingManagersByOrgGuid(testOrganizationGuid, "bad-token")
    ...
    It("returns an error when the Cloud Controller returns a 404 status code", func() {
        _, err := cloudController.GetBillingManagersByOrgGuid("my-nonexistant-guid", testUAAToken)
