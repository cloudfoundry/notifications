...
var _ = Describe("GetAuditorsByOrgGuid", func() {
    ...
    BeforeEach(func() {
        ...
        CCServer = httptest.NewServer(AuditorsEndpoint)
    ...
    It("returns a list of auditors for the given organization guid", func() {
        users, err := cloudController.GetAuditorsByOrgGuid(testOrganizationGuid, testUAAToken)
        if err != nil {
            panic(err)
        }
    ...
    It("returns an error when the Cloud Controller returns a 400, or 500 status code", func() {
        _, err := cloudController.GetAuditorsByOrgGuid(testOrganizationGuid, "bad-token")
    ...
    It("returns an error when the Cloud Controller returns a 404 status code", func() {
        _, err := cloudController.GetAuditorsByOrgGuid("my-nonexistant-guid", testUAAToken)
